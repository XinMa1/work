...
TINY.editor=function(){
	...
	edit.prototype.action=function(cmd,val,ie){
		if(ie&&!this.ie){
			...
			switch(cmd){
				case 'uploadfile':
					if(!window.open(uploadexc,'newwindow', 'height=240, width=460,toolbar=no, menubar=no, scrollbars=no,resizable=no,status=no,left=400,top=200')){
	...
	edit.prototype.insert=function(pro,msg,cmd){
		if(cmd == "insertimage"){
			window._tmp_edit_obj = this.e;
			document.getElementById("toggle_select_thumbnail").click();
			return;
		}
